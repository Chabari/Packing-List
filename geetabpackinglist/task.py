...
def before_submit(doc, method):
    if not doc.items:
        frappe.throw("Failed. Enter items before submitting.")
        
    for itm in doc.items:
        if itm.rate < 1 and doc.is_return == 0:
            frappe.throw(f"Failed. Enter rate for the following item {itm.item_code}.")
        
        if itm.qty < 1 and doc.is_return == 0:
            frappe.throw(f"Failed. Enter quantity for the following item {itm.item_code}.")
